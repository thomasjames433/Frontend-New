  ...
  return (
    <nav className="absolute top-0 left-0 w-full bg-opacity-50 text-white px-8 py-4 flex justify-between items-center z-[10000]">
      ...
      <ul className={`md:flex space-x-8 text-lg mx-3 
        ${isOpen ? "absolute top-16 left-0 w-full bg-black bg-opacity-80 p-10 flex flex-col items-start space-y-4 pl-6 h-[250px]" : "hidden md:flex"}`}>
        ...
        <li className="cursor-pointer text-xl hover:text-gray-300 px-6" onClick={() => scrollToSection(artistsRef)}>Meet The Ambassador</li>
